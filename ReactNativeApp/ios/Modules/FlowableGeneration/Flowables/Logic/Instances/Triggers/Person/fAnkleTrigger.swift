...
class fAnkleTrigger: BaseLogic {
  
  var output: BasePort<[DetectionLocation]> = BasePort(name: "ankleDetLocs", isDynamic: false, isOptional: false, isEndPort: false)
  ...
  private func connectToObjects() {
    if self.uid.lowercased().contains("l") {
      objectConnectedTo = GameContext.getContext()?.personState?.leftLeg?.ankle
    } else {
      objectConnectedTo = GameContext.getContext()?.personState?.rightLeg?.ankle
    }
  }
