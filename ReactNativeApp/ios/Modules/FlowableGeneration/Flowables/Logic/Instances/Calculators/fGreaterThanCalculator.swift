...
class fGreaterThanCalculator: BaseLogic {
  ...
  override func process() -> Bool {
    let val: Bool = (self.v1.get()?.getDouble() ?? -100000 > self.v2.get()?.getDouble() ?? 1000000)
    Logger.shared.log(logType: .DEBUG, message: "\(val): \(self.v1.get()?.getDouble() ?? 0) > \(self.v2.get()?.getDouble() ?? 1)")
