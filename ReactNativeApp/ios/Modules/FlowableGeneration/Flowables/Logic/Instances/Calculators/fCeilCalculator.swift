...
class fCeilCalculator: BaseLogic {
  
  var inputs: BasePort<FlblPrimitiveType> = BasePort(name: "input", isDynamic: false, isOptional: false, isEndPort: false)
  ...
  override func process() -> Bool {
    output.set(val: ceil(self.inputs.get()?.getDouble() ?? 0))
    return true
  }
